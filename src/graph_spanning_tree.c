...
void graph_prim(struct graph* graph, struct graph* spanning_tree) {
    ...
    struct vertex* start = graph_vertex(graph, vec_get(graph->vertices, 0));
    for(int i = 0; i < vec_len(start->out); i++) {
        struct vertex* neighbour = vec_get(start->out, i);
        struct edge* edge = graph_edge_between(graph, start->value, neighbour->value);
    ...
    while(1) {
        ...
        for(int i = 0; i < vec_len(to_vertex->out); i++) {
            struct vertex* neighbour = vec_get(to_vertex->out, i);
            struct edge* edge = graph_edge_between(graph, to_vertex->value, neighbour->value);
            priority_queue_nq(q, edge, edge->weight);
        }
