...
int successor_graph_binary_lift(struct graph* graph, int vertex_i, int limit, int** succ) {
    struct vertex* v = graph_vertex(graph, graph_vertex_from_i(graph, vertex_i));

    if(succ[log_2(limit)][vertex_i] != -1) return succ[log_2(limit)][vertex_i];

    succ[log_2(limit)][vertex_i] = limit == 1 
        ? vec_get(v->out, 0)->i
        : successor_graph_binary_lift(
            graph,
            successor_graph_binary_lift(graph, v->i, limit/2, succ), limit/2, succ
        );
...
void successor_graph_precalculate_succ(struct graph* successor_graph, int limit, int** succ) {
    for(int i = limit; i >= 0; i--) {
        for(int j = 0; j < graph_vertices_len(successor_graph); j++) {
            succ[i][j] = -1;
        }
    }
    for(int i = 0; i < graph_vertices_len(successor_graph); i++) {
        successor_graph_binary_lift(successor_graph, i, limit, succ);
    }
}
